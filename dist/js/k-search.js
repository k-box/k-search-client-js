...
var format = {
    ...
    duration: function (duration) {
        ...
        var filtered_matches = lodash_filter(matches, function(v){
            return (v !== "0." && v !== "0:");
        });
...
function KSearchClient(options) {
    ...
    function Data(result) {
        ...
        if (result.properties.video && result.properties.video.streaming && result.properties.video.streaming.length > 0) {
            streamings = lodash_transform(result.properties.video.streaming, function (result, stream) {
        ...
        this.audio = {
            languages: result.properties.audio ? lodash_map(result.properties.audio, function (audio) {
    ...
    function SearchResults(request, original) {
        ...
        lodash_foreach(results.items, function (v) {
    ...
    function convertAggregationsToShorthand(aggregations){
        ...
        var transformed = lodash_transform(aggregations, function (result, value, aggregationName) {
...
var kSearchClientModule = KSearchClient;
...
window.ksearch = function (options) {
    ...
    var SearchClient = new kSearchClientModule({url: _options.url, token: _options.token});
    ...
    function search(term, options) {
        ...
        SearchClient.find(requestData).then(function (value) {
            ...
            if(value.results.total > 0){
                lodash_foreach(value.data, function(el){
